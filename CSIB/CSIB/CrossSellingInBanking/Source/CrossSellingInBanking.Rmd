...
```{r}
df_client <- df_client %>% 
  mutate(across(c("client_id", "district_id", "sex"), as.factor))
str(df_client)
```
...
df_disposition <- df_disposition %>% 
  mutate(across(c("disp_id":"type"), as.factor))
str(df_disposition)
...
Klient mit dem District vereint
```{r}
client_district_joined <- df_client %>% 
  full_join(df_district, by = "district_id")
```

District mit dem Account vereint, Datum in einzelne Spalten aufgeteilt und die Reihenfolge der Kolonnen angepasst.
```{r}
district_full_join <- client_district_joined %>% 
  full_join(df_account, by = "district_id")
...
```{r}
disp_full_join <- df_disposition %>% 
  full_join(df_credit_card, by = "disp_id") %>% 
  full_join(df_account, by = "account_id")

disp_full_join <- disp_full_join %>% 
  mutate(year = year(disp_full_join$date)) %>% 
  mutate(month = month(disp_full_join$date)) %>% 
  mutate(day = day(disp_full_join$date))

str(disp_full_join)
summary(disp_full_join)
```
