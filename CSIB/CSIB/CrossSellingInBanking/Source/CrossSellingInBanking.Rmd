...
```{r}
# rm(cardwidthdrawals, cardwidthrawals_4, cashwidthdrawals, cashwidthdrawals_10, date_complete, trans_correlation, transactions)
```
...
```{r}
# Anzahl Monate
incomes_n <- df_transaction_complete %>% 
  group_by(
    account_id,
    year(date),
    month(date)
    ) %>% 
  summarise(sum = sum(incomes)) %>% 
  filter(sum > 0) %>% 
  count()

# Einkommen pro Jahr und aufgerechnet auf den Monat
incomes_per_year <- df_transaction_complete %>%
  group_by(
    account_id,
    year(date)
  ) %>%
  summarise(income_sum = sum(incomes)) %>% 
  filter(income_sum > 0) %>% 
  left_join(incomes_n, by = c("account_id", "year(date)")) %>% 
  mutate(salary_per_month = round((income_sum / n), digits = 0)) %>% 
  mutate(salary_per_year = round((income_sum / n * 12), digits = 0)) %>% 
  select(-c(income_sum, n))
  
# Anzahl Monate
outgoes_n <- df_transaction_complete %>% 
  group_by(
    account_id,
    year(date),
    month(date)
    ) %>% 
  summarise(sum = sum(outgoes)) %>% 
  filter(sum > 0) %>% 
  count()

# Geldabfluss pro Jahr und aufgerechnet pro Monat
outgoes_per_year <- df_transaction_complete %>%
  group_by(
    account_id,
    year(date)
  ) %>%
  summarise(outgoes_sum = sum(outgoes)) %>% 
  filter(outgoes_sum > 0) %>% 
  left_join(outgoes_n, by = c("account_id", "year(date)")) %>% 
  mutate(outgoes_per_month = round((outgoes_sum / n), digits = 0)) %>% 
  mutate(outgoes_per_year = round((outgoes_sum / n * 12), digits = 0)) %>% 
  select(-c(outgoes_sum, n))

# Einkommen und Geldabflüsse mit Vermögensentwicklung
in_out <- incomes_per_year %>% 
  left_join(outgoes_per_year, by = c("account_id", "year(date)")) %>% 
  na.replace(., 0) %>% 
  mutate(diff = round((salary_per_year - outgoes_per_year), digits = 0))


# Vermögensentwicklung über die gesamte Zeit
assets_development <- in_out %>% 
  group_by(account_id) %>% 
  summarise(assets_development_sum = round(sum(diff), digits = 0)) 

# Anfügen der Vermögensentwicklung an das konsolidierte Data Frame
df_cons <-  df_cons %>% 
  left_join(assets_development, by = "account_id")
...
# Jahreseinkommen im Durchschnitt
salary_avg <- in_out %>% 
  group_by(account_id) %>% 
  summarise(salary_sum = sum(salary_per_year)) %>% 
  left_join(n, by = "account_id") %>% 
  mutate(salary_avg = round((salary_sum / n), digits = 0)) %>% 
  select(account_id, salary_avg)

# Anfügen des Jahreseinkommens an das konsolidierte Data Frame
df_cons <- df_cons %>% 
  left_join(salary_avg, by = "account_id")

head(assets_development)
head(salary_avg)
head(in_out)
rm(n, outgoes_per_year, incomes_per_year, outgoes_n, incomes_n)
...
ggplot(df_transaction_type_count, aes(fill = characterization, x = quarter, y = count)) +
  ...
  facet_wrap(vars(characterization))
```
...
#### x. Verteilung der Kundschaft auf die Distrikte in Tschechien. (Aaron)
