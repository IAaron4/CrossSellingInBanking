...
outgoes <- outgoes %>% 
  mutate(
    period = case_when(
      (trans_month <= 3) ~ "1. QUARTER",
      (trans_month >= 4 & trans_month <= 6) ~ "2. QUARTER",
      (trans_month >= 7 & trans_month <= 9) ~ "3. QUARTER",
      (trans_month > 9) ~ "4. QUARTER"
    ),
    across(
      where(is.character), 
      as.factor
...
card_withdrawal %>% 
  ...
  head
```
Die meisten Kreditkartenbezüge sind unter 4000 ? Welche Währung??? Tschechische Krone?
```{r}
card_withdrawal %>% 
  filter(
    trans_amount == 8000
