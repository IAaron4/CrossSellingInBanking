...
```{r}
# create Scatterplot of all Contract types with duaration and Amount of loan

# create group for every loan type
loan_fin_notpayed <- df_cons %>%
    filter(loan_status == "CONTRACT FINISHED UNPAID") 

loan_fin_payed <- df_cons %>%
    filter(loan_status == "CONTRACT FINISHED PAYED")

loan_opn_ok <- df_cons %>%
    filter(loan_status == "CONTRACT OPEN OK")

loan_opn_indept <- df_cons %>%
    filter(loan_status == "CONTRACT OPEN INDEBT")
```
...
loanplot3 <- df_cons %>%
  ...
  theme(axis.text.x=element_blank(),
        axis.ticks.x=element_blank()) +
  labs(title = "Age at the beginning of Loan", 
           subtitle = "", 
           caption = "",
           y = "", x = "Years") +
  coord_flip()
...
nested_loan_plot <- ((loanplot1|loanplot2)|loanplot3) 
nested_loan_plot
...
Höhe der unbezahlten Kredite
```{r}
df_cons %>%
  filter(loan_status == "CONTRACT FINISHED UNPAID") %>%
  ggplot(df_cons, mapping = aes(x = loan_total_amount, y = 0, fill = "#CD3700")) +
  geom_boxplot() +
  theme(axis.text.x=element_blank(),
        axis.ticks.x=element_blank(),
        legend.position = "none") +
  scale_x_continuous(breaks = scales::breaks_pretty(10)) +
  labs(title = "Amount of unpaid loans", 
           subtitle = "Contract finished unpaid", 
           caption = "Source: PKDD’99 Discovery Challenge",
           y = "", x = "Value [CZK] ") +
  ylim(-0.7,0.7) +
  coord_flip()
...
Die Abbildung oben zeigt, wie hoch die Kredite waren welche durch die Kunden nicht zurückbezahlt werden konnten. Dabei handelt es sich um insgesamt um 31 Kredite zwischen 30'000 und 464'000 Tschechischen Kronen. Wobei in der Abbildung ersichtlich ist, dass es sich bei dem Kredit über 464'000 Tschechischen Kronen um einen Ausreisser handelt. Dies wird auch durch den median welecher bei 96'000 Tschechischen Kronen liegt, ersichtlich.
