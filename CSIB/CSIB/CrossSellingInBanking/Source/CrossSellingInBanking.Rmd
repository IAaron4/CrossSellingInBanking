...
### 2.1. Bargeldbezüge ohne Angabe des Verwendungszwecks
...
#### 2.1.a. Varialbelbeschreibung (Levels)

[^96] Folgende Levels sind vorhanden:
...
#### 2.1.b. Bargeldbezüge über 10'000 CZK
...
#### Ziel
Wenn ein Kunde bereits ein Girokonto hat aber noch nicht mit einer Kreditkarte verbunden ist,  ein Angebot für die Kreditkarte unterbreiten. Bei Neueröffnung eines Girokontos die Kreditkarte gleich mitanbieten.
...
```
Nur 892 Accounts von gesamthaft 4500 sind mit einer Kreditkarte verbunden.

Erstellen einer neuen Spalte "has_creditcard" mit dem logischen Wert TRUE wenn ein Konto mit einer Kreditkarte verbunden ist.
```{r}
df_cons <- df_cons %>% 
...
Konten mit Kreditkarte, welche aber die Kreditkarte noch nicht eingesetzt haben.
```{r}
cc_without_trans <- cc %>% 
...
```
An der Verteilung auf die Jahre ist sehr gut zu erkennen, dass die Kreditkarte immer mehr an Beliebtheit gewonnen hat. Nun ist der richtige Zeitpunkt gekommen, um die Kreditkarte bei der breiten Masse der Kundschaft beliebt zu machen.

Wieviele Jahre sind verstrichen nach Kontoeröffnung bis zum ersten Einsatz der Kreditkarte?
```{r}
cc_first %>% 
  group_by(diff_year) %>% 
  count() %>% 
...
Bei Kreditkartenbezügen sollte kein Verwendungszweck vermerkt sein. Zudem verschaffen wir uns einen Überblick über die Höhe der jeweiligen Bezüge.
```{r}
...
```
Die Accounts mit Kreditkartenbezügen sind über das ganze Land verteilt. Die Region nord Moravia fällt durch die häufigsten  und south Bohemia durch die wenigsten Bezüge auf.
...
#### 2.1.d. Top 3 der höchsten Kreditkartenbezüge
```{r}
cardwidthdrawals %>% 
  top_n(3, amount)
...
```
#### 2.1.e. Kriterien an den Kunden für das Angebot einer Kreditkarte

Das Angebot einer Kreditkarte sollte für den Kunden sowie für die Bank einen Mehrwert bedeuten. Darum ist es sehr wichtig, die Kriterien, welche einen "guten" Kreditkartenkunde ausmacht, festzulegen. Die bestehende Kundschaft wird dann auf diese Kriterien hin geprüft und im Anschluss können gezielt Angebote an die Kundschaft getragen werden. Für Neukunden werden diese Beurteilungskriterien soweit als möglich angewendet.
...
- Vermögensentwicklung nicht im Minus


#### 2.1.f. Kontoart

Die uns zugrunde liegenden Daten sagen leider nichts über die Kontoart aus. Deshalb muss dies mittels Analyse bestimmt werden.
Folgende Gedanken dazu: ein Girokonto hat regelmässige Transaktionen. Es gibt im Durchschnitt mindestens je eine Transaktion (in und out) pro Monat.

Erstellen einer neuen Spalte "is_checking_account" mit dem logischen Wert TRUE wenn es sich beim Konto um ein Girokonto handelt.
...
#### 2.1.g. regelmässiges Einkommen

Um die Kreditkartenausgaben decken zu können braucht der Kunde ein regelmässiges Einkommen, welches über dem Existenzminimum liegt. Wir gehen davon aus, dass Mitte der 90er Jahre das Existenzminimum bei 750 CZK pro Monat lag. 

Erstellen einer neuen Spalte "regular_income" mit dem logischen Wert TRUE wenn auf dem Konto regelmässig Beträge über dem Existenzminimum vorhanden sind.
```{r}
# Filtern nach Accounts ohne regelmässige monatliche Einkommen
no_reg_income <- df_transaction_complete %>% 
  ...
  summarise(incomes_monthly = sum(incomes)) %>% 
  mutate(regular_income = ifelse(incomes_monthly > 750, TRUE, FALSE)) %>% 
  group_by(account_id) %>% 
  count(regular_income) %>% 
  filter(regular_income == FALSE,
         n > 1)

# Erstellen der neuen Spalte "regular_income"
reg_income <- df_transaction_complete %>% 
  mutate(regular_income = ifelse(account_id %in% no_reg_income$account_id, FALSE, TRUE)) %>% 
  select(
    account_id,
    regular_income
  ) %>% 
...
df_cons <- df_cons %>% 
  left_join(reg_income, by = "account_id")

# save(df_cons, file = "df_cons.RData")
rm(reg_income, no_reg_income)

```

Überprüfung anhand des Accounts mit dem unregelmässigsten Einkommen
```{r}
df_transaction_complete %>% 
  filter(account_id == 4059) %>% 
  drop_na(trans_id) %>% 
  tail(5)

```
#### 2.1.h. Vermögensentwicklung

Die Vermögensentwicklung darf nicht negativ sein, da ansonsten die Kredite nicht mehr abbezahlt werden können.
...
die gesamte Zeit berechnet. 

[^101]: Erstellt von Léonie Bécheiraz

Durchschnittliche Vermögensentwicklung über die gesamte Laufzeit. 
Erstellen einer neuen Spalte "assets_dev_pos" mit dem logischen Wert TRUE wenn das Konto eine positive Vermögensentwicklung über die gesamte Laufzeit aufweist.
```{r}
...
```{r}
# List of customers for credit card offers
cust_cc_offer_lst <- df_cons %>% 
 filter(
...
nrow(cust_cc_offer_lst)

```
2503 Kunden erfüllen die Voraussetzung für ein Kreditkartenangebot und haben bis jetzt noch keine Kreditkarte mit dem Girokonto verbunden.

#### Art der Kontaktaufnahme dieser Kunden
Wir leben in einem Zeitalter, in dem die Kunden gerne persönlich in der Filiale beraten werden. Diese persönliche Beratung bietet sich vor allem dann an, wenn der Kunde im selben Distrikt wohnt wie seine Filiale angesiedelt ist.

Erstellen einer neuen Spalte "consultation_on_site" mit dem logischen Wert TRUE wenn das Konto eine positive Vermögensentwicklung über die gesamte Laufzeit aufweist.
```{r}
...
cust_cc_offer_lst <- df_cons %>% 
  filter(account_id %in% cust_cc_offer_lst$account_id) %>% 
...
[^106] Die jährliche Vermögensentwicklung bewegt sich in den Jahren 1993-1998 zwischen -235716 und 716328.

[^106]: Erstellt von Léonie Bécheiraz
```{r}
df_cons %>% 
  mutate(assets_dev_avg = assets_dev_avg / 1000) %>%
  ggplot(aes(assets_dev_avg)) +
  geom_density() +
  theme_minimal() +
    theme(
      legend.position = "none",
...
geringeren negativen Entwicklung deutlich von den folgenden Jahren ab. Ebenso ist
das Jahr 1998 am stärksten mit negativen Beträgen belastet. Erstaundlich
...
# Irgendwas mit Attribut sallary_group und incomes_avg erstellen
