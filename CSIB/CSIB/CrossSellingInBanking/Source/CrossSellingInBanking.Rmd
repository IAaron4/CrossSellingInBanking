...
Als erstes werden die Kontostände per Ende jeden Monats berechnet. Dazu müssen die entstehenden NA's mit der Zahl Null ersetzt werden, da ja bei diesen Kontoständen sowie Geldein- und ausgängen 0 CZK vorhanden sind.
```{r}
...
balance_per_month_total <- transaction_cashflow_per_month_and_year_total %>% 
  select(
    account_id,
    year,
    month,
...
balance_sheet_per_years_total <- years_total %>% 
    select(
      account_id,
      year,
      balance_sheet
...
if (!grepl("balance_sheet_total.", names(balance_sheet_per_years_total)[2])) {
names(balance_sheet_per_years_total) <- paste0("balance_sheet_total.", names(balance_sheet_per_years_total))
}
...
Neu erstellte Spalte mit den Bilanzen an das bestehende df_mod anfügen.
```{r}
df_mod <- left_join(df_mod, balance_sheet_per_years_total, by = c("account.id" = "balance_sheet_total.account_id"))
